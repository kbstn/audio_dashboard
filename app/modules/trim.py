...
class TrimModule(BaseModule):
    ...
    def render_ui(self) -> None:
        ...
        try:
            ...
            with col2:
                max_end = min(duration, self.settings.start_time + 3600)  # Cap at 1 hour from start
                self.settings.end_time = st.slider(
                    "End Time",
                    min_value=min(self.settings.start_time + 0.1, max_end),
                    max_value=max_end,
                    value=min(duration, self.settings.start_time + 30),  # Default 30s clip
                    step=0.1,
                    format="%.1f s"
                )
            
            # Display selection info
            st.write(f"**Selection:** {self._format_time(self.settings.start_time)} - {self._format_time(self.settings.end_time)} "
            ...
            if st.button("Trim Audio"):
                with st.spinner("Trimming audio..."):
                    try:
                        ...
                        st.success("Audio trimmed successfully! The trimmed file has been added to your file list.")
